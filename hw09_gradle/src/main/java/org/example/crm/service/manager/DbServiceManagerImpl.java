package org.example.crm.service.manager;
...
public class DbServiceManagerImpl implements DBServiceManager {
    ...
    public Manager saveManager(Manager manager) {
        return transactionRunner.doInTransaction(connection -> {
            if (manager.getNo() == null) {
                ...
                log.info("созданный объект: {}", createdManager);
                return createdManager;
            }
            managerDataTemplate.update(connection, manager);
            log.info("удаленный объект: {}", manager);
            return manager;
        });
    }
    ...
    public Optional<Manager> getManager(long no) {
        return transactionRunner.doInTransaction(connection -> {
            ...
            log.info("объект с id= {} : {}", no, managerOptional);
    ...
    public List<Manager> findAll() {
        return transactionRunner.doInTransaction(connection -> {
            ...
            log.info("список объектов :{}", managerList);
            return managerList;
        });
    }
}
